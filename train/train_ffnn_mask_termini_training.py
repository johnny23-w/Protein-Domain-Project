...
def create_parser():
    ...
    parser.add_argument('-hidden_dim', metavar='--hidden_dim', type=int, help='fully connected layer hidden dimension')
...
class Network(nn.Module):
    def __init__(self, hidden_dim):
        super().__init__()
        self.fc = nn.Sequential(nn.Linear(1024, hidden_dim),
                                nn.ReLU(),
...
def run(args):
    ...
    with h5py.File(train_embedding_file) as train_embs:
        with h5py.File(valid_embedding_file) as valid_embs:
            ...
            model = Network(hidden_dim=hidden_dim)
