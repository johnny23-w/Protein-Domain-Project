...
def create_parser():
    ...
    parser.add_argument('-termini_weight', metavar='--termini_weight', type=float, help='downweighting to apply at termini')
...
def train_fn(model, train_loader, valid_loader, N_epochs, l_rate, weight_decay ,early_stopping, output_dir, termini_weight, boundary_weight):
    ...
    for idx, epoch in enumerate(range(N_epochs)):
        ...
        for batch in loop:
            ...
            n_bounds = batch[4].to(device)
            c_bounds = batch[5].to(device)

            exterior_weighting = torch.ones_like(lbls).float()
            for i in range(len(lens)):
                exterior_weighting[i, :n_bounds[i]+10] *= termini_weight
                exterior_weighting[i, c_bounds[i]-1-10:] *= termini_weight

            opt.zero_grad()

            outputs = model(embs, mask).squeeze()

            loss = (criterion(outputs, lbls.float())*mask*exterior_weighting).sum() / (len(lens) * 322.65)
            loss.backward()
...
def run(args):
    ...
    termini_weight = args.termini_weight
